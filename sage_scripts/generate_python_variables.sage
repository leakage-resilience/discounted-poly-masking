...
# Save arrays in python format
filepath=f"../ChipWhisperer/poly_masking_setup/poly_masking_parameters_d{degree}e{faults}k{num_secrets_per_encoding}.py"
print("Saving to file...")

with open(filepath, 'w') as f:
    f.write("# SPDX-License-Identifier: MIT\n")
    f.write("# Copyright 2025-2026 Max Planck Institute for Security and Privacy (MPI-SP), University of Luebeck\n")
    f.write(f"secrets_supports = [")
    alpha_str = ", ".join(str(s.integer_representation()) for s in secrets_supports)
    f.write(alpha_str)
    f.write("]\n")

    f.write(f"shares_supports = [")
