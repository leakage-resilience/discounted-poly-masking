...
void aes_indep_init(void) { HW_AES128_Init(); }

void aes_indep_key(uint8_t *key) { HW_AES128_LoadKey(key); }

void aes_indep_enc_pretrigger(uint8_t *pt) { HW_AES128_Enc_pretrigger(pt); }

void aes_indep_enc_posttrigger(uint8_t *pt) { HW_AES128_Enc_posttrigger(pt); }

void aes_indep_enc(uint8_t *pt) { HW_AES128_Enc(pt); }
...
void aes_indep_key(uint8_t *key) { aes128_init(key, &ctx); }
...
/*  This is the AES RSM 256 encryption function that call the generic AES RSM
 * encryption core*/
...
void aes_indep_key(uint8_t *key) { aes256_init(key, &ctx); }
...
void aes_indep_key(uint8_t *key) { AES128_ECB_indp_setkey(key); }

void aes_indep_enc(uint8_t *pt) { AES128_ECB_indp_crypto(pt); }
...
void aes_indep_key(uint8_t *key) { mbedtls_aes_setkey_enc(&ctx, key, 128); }

void aes_indep_enc(uint8_t *pt) {
  mbedtls_aes_crypt_ecb(&ctx, MBEDTLS_AES_ENCRYPT, pt,
                        pt); /* encrypting the data block */
}
...
void aes_indep_enc(uint8_t *pt) { asm_maskedAES128enc(); }
...
void aes_indep_key(uint8_t *key) {
  ...
  for (i = 0; i < AESKeySize; i++)
    temp_key[i] = key[i];
#else
  aes(MODE_KEYINIT | mask_modes, &aes_ctx, key, NULL /* plaintext */,
      NULL /* encrypted text */, aes_mask, key_mask);
#endif
}
...
void aes_indep_enc(uint8_t *pt) {
#ifdef TRIG_BEFORE_KS
  aes(MODE_KEYINIT | MODE_AESINIT_ENC | MODE_ENC | mask_modes, &aes_ctx,
      temp_key, pt /* plaintext */, pt /* encrypted text */, aes_mask,
      key_mask);
#else
  aes(MODE_AESINIT_ENC | MODE_ENC | mask_modes, &aes_ctx, NULL /* key */,
      pt /* plaintext */, pt /* encryted text */, aes_mask, key_mask);
#endif
}
...
bitslice_t get_random_bitslice(void) { return get_rand(); }
